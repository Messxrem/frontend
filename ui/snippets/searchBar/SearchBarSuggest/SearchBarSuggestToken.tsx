...
const SearchBarSuggestToken = ({ data, isMobile, searchTerm }: Props) => {
  ...
  const additionalInfo = (
    <Text overflow="hidden" whiteSpace="nowrap" fontWeight={ 700 }>
      { data.token_type === 'DNC-20' && data.exchange_rate && `$${ Number(data.exchange_rate).toLocaleString() }` }
      { data.token_type !== 'DNC-20' && data.total_supply && `Items ${ Number(data.total_supply).toLocaleString() }` }
    </Text>
  );

  if (isMobile) {
    const templateCols = `1fr
    ${ (data.token_type === 'DNC-20' && data.exchange_rate) || (data.token_type !== 'DNC-20' && data.total_supply) ? ' auto' : '' }`;
